...
public class EvaluationService {
	...
	public String acronym(String phrase) {
		...
		StringBuilder arr = new StringBuilder(phrase);

		for (int i = 0; i < phrase.length() - 1; i++) {
			if (isLetter) {
				isLetter = false;
				arr.setCharAt(totLetters++, arr.charAt(i));
			}
			if (arr.charAt(i) == ' ')
				isLetter = true;
		}
		char[] temp = new char[totLetters];
	...
	public int getScrabbleScore(String string) {
		...
		for (int i = 0; i < string.length(); i++) {
			if (onepoint.indexOf(string.charAt(i)) >= 0)
				score++;
			if (twopoint.indexOf(string.charAt(i)) >= 0)
				score += 2;
			if (threepoint.indexOf(string.charAt(i)) >= 0)
				score += 3;
			if (fourpoint.indexOf(string.charAt(i)) >= 0)
				score += 4;
			if (fivepoint.indexOf(string.charAt(i)) >= 0)
				score += 5;
			if (eightpoint.indexOf(string.charAt(i)) >= 0)
				score += 8;
			if (tenpoint.indexOf(string.charAt(i)) >= 0)
				score += 10;
		}
		return score;
	}
	...
	public String cleanPhoneNumber(String string) {
		// +1 (613)-995-0253 613-995-0253 1 613 995 0253 613.995.0253
		StringBuilder worker = new StringBuilder(string);
		int lastindexchecked = 0;
		if (worker.charAt(0) == '+') {
			worker.setCharAt(0, ' ');
			worker.setCharAt(1, ' ');
		} else if (worker.charAt(0) == '1') {
			worker.setCharAt(0, ' ');
		}
		for (int i = 0; i < string.length(); i++) {
			if (worker.charAt(i) >= '0' && worker.charAt(i) <= '9') {
				worker.setCharAt(lastindexchecked++, worker.charAt(i));
			}
		}
		String result = worker.substring(0, 10);
		return result;
	}
	...
	public Map<String, Integer> wordCount(String string) {
		...
		Map<String, Integer> wordcontainer = new HashMap<String, Integer>();
		int letterstart = 0;
		StringBuilder worker = new StringBuilder(string);
		for (int i = 0; i < string.length(); i++) {
			if (worker.charAt(i) == ' ' || i == string.length() - 1) {
				// we completed a word
				String temp;
				if (i + 1 == string.length())
					temp = worker.substring(letterstart, i + 1);
				else
					temp = worker.substring(letterstart, i);
				// System.out.println(temp);

				// is there this word in map
				if (wordcontainer.containsKey(temp)) {
					int count = wordcontainer.get(temp);
					wordcontainer.put(temp, count + 1); // updates
				} else {
					wordcontainer.put(temp, 1);
				}
				letterstart = i + 1; // +1 because its a ' '
			}
